...
<template>
  ...
  <div class="ztree_content_wrap" v-if='treeDataSource.length>0'>
    <div class="zTreeDemoBackground left">
      <ul class="ztree" v-for="j of treeDataSource.length" :key="j">
        <ztree-item v-for='(m,i) in treeDataSource[j]' :key='i' :model.sync="m" :num.sync='i'
                    root='0' :nodes.sync='treeDataSource[j].length' :ischeck='isCheck' :callback='func'
                    :expandfunc='expand' :cxtmenufunc='contextmenu' :trees.sync='treeDataSource[j]'></ztree-item>
      </ul>
    </div>
  </div>
</template>
